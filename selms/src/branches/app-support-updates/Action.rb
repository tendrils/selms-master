...
class Action
	...
	class Email < Action::Base
		...
		def produce_reports(processed_hosts)
			...
			processed_hosts.each { |host, _| # each host we have logs to report for
				# skip unless we have something to report

				c = 0
				host.recs.each { |_, recs |
					c += recs.size
				}
				...
				if host.count.size > 0 # more than the default counts
					summ = []
					host.count.sort.each { |_, v|
						case v.label
			...
			if $options['no_mail'] && !$options['mail_to']
				$options['outfile'] = '-'
			end
		...
		def send_async(server, from, to, data); end
...
class Net::SMTP # :nodoc:
	...
	unless instance_methods.include? 'reset'
		##
...
class Mail < Net::SMTP
	...
	def initialize(server, from, *args)
		...
		if block_given?
			begin
				return yield(self.start(server, *args))
			ensure
				finish
			end
		else
