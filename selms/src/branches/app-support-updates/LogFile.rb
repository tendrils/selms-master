...
#      puts "Gets: #{l} #{raw} #{continuation}" if  $options['debug.gets'] 
		...
		time = 0  # time of first duplicate
		...
		puts "Gets: initial '#{initial}', index #{file_index}" if $options['debug.gets']
			...
			begin  # loop while records are the same
				...
				unless initial # select next file with earliest log record
					file_index = 0
				...
				begin   # loop to collaspe repeated records and handle records with newlines...
					...
					if initial && ! (defined? @rec[file_index].data) # corrupt offset value?
						count = 0
		...
		r.count = count
		if count > 1
			unless r.orec # something broken in the parsing
...
# have to drop the look a head!!
	def close_lf( lf = nil )
		...
		unless $options['no_write_offset'] ||  $options['no_offset']
			File::open(off_n, 'w') { |o|
				o.puts offset.to_s
			}
		end
