...
class RunOutMemory < RuntimeError ; end
...
OptionParser.new { |opts|
  opts.on( '--debug=DEBUG', debug_opts , String) {
			|val| $options['debug.'+val] = true }
  opts.on( '-t', '--task=RUN_TYPE', %w( realtime, periodic, daily, weekly, monthly), String) {
			|val| $options['run_type'] = val }
  opts.on('-o', '--offset=OFFSET', String, 'use ARG as offset file suffix') {
			|val| $options['offset'] = val }
  opts.on('-O', '--no_write_offset' ) {
			|val| $options['no_write_offset'] = val }
  opts.on('--no_offset' ) {
			|val| $options['no_offset'] = val
  					$options['no_write_offset'] = val  }
  opts.on('-m', '--mail_to=MAIL_TO', String, 'send all mail to ARG') {
			|val| $options['mail_to'] = val }
  opts.on('--mail_from=MAIL_FROM', String, 'address to use as from for email') {
			|val| $options['mail_from'] = val }
  opts.on('-L', '--lock=LOCK_FILE', String, 'name of lock file') {
			|val| $options['lock'] = val }
  opts.on('--mail_server=MAIL_SERVER', String, 'send mail via ARG') {
			|val| $options['mail_server'] = val }
  opts.on('--mail_subject=MAIL_SUBJECT', String, 'prefix ARG to mail subject line') {
			|val| $options['mail_subject'] = val }
  opts.on('-M', "Don't send mail to listed contacts") {
			|val| $options['no_mail'] = val }
  opts.on('-s', '--summary=SUMM_TO', String, 'send summary to ARG') {
			|val| $options['summ_to'] = val }
  opts.on('-u', '--ignore_unk_hosts', "Ignore hosts that we don't have explicit defs for") {
			|_| 	$options['ignore_unk_hosts'] = true }
  opts.on('-t', '--time-hosts=TIME_THRES', Integer, 'Print processing times for each host, implies host') {
			|val| $options['time-hosts'] = val }
  opts.on('--timeout=TIMEOUT', Integer, 'limit of elapsed time to spend on any one file') {
			|val| $options['timeout'] = val }
  opts.on('-h', '--host=HOSTNAME', 'run just for this host') {
			|val| $options['one_host'] = val.downcase }
  opts.on('-f', '--file=FILENAME', 'run just for this file (daemon, auth, etc)') {
			|val| $options['file'] = val }
  opts.on('--one_file=FILENAME', 'run just for this file') {
			|val| $options['one_file'] = val }
  opts.on('--log_type=PLUGIN', 'explicitly specify plugin -- use with -f') {
			|val| $options['log_type'] = val }
  ...
  opts.on('--date=DAY', String, 'Run for this day') {
			|val| $options['date'] = val
  ...
  case $options['run_type']
    ...
    when 'realtime'
      ...
      while rt.watch_it do
        ...
        if $options['rt_socket']  !=  new_conf.vars['rt_socket']
        ...
        if new_rt.report.code != rt.report_code or
            new_rt.scan.code != rt.scan_code # changes affect RT
          old_rt = rt
          rt = new_rt
        end
        #      end
        if ! rt.thread or ! rt.thread.alive?
          rt.run_it
        end
        if old_rt && old_rt.thread.alive?
